...
<nav class="navbar navbar-expand-sm navbar-dark bg-dark">
	...
	<?php
			...
			$users = mysqli_query($connection, "SELECT * FROM `users` ORDER BY `id` DESC LIMIT 100");

			if (!$users) {
				echo "<h1>Статей не завезли</h1>";
			}else
			{
			while ( $user = mysqli_fetch_assoc($users) ) 
			{
				?> 
				<div method="POST" action="administration.php" class="jumbotron" style="background: white;">
			 	 <h2 class="lead">Ник пользователя - <? 

 				 $admin = "";

			 	 if ($user['username'] == 'georgejornd' | $user['username'] == "Shigen" | $user['username'] == "Stuff") 
			 	 {
			 	 	$admin = " (админ)";
			 	 }
