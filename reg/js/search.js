    ...
    function find() {
        ...
        if (s.length == 1 & s[0] == '') {
            ...
            if (ei == statsDraw.length) {
                search_result.insertAdjacentHTML("afterbegin", '<h1 id="search_result" class="card"><div class="card-body"><p class="card-text">По вашему запросу ничего не найдено :(</h1><br><h1 style="margin-left: 50px;">Совет по поиску, если вы не находите что вам нужно сделать:</h5><br><h2 style="margin-left: 50px;">-Следите за регистром, возможно проблема в этом. Просто поменяйте регискр первой буквы </h2>')
            }
            if (night) {
